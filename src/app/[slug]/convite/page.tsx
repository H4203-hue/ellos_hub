...
export default function WorkspaceConvitePage() {
  ...
  return (
    <div className="min-h-screen max-h-screen bg-[#0F223D] text-slate-100 flex flex-col justify-between p-3 sm:p-4 overflow-hidden font-sans no-scrollbar">
      {/* Header Limpo com Logo do Workspace */}
      <header className="max-w-md mx-auto w-full pt-1 pb-1 flex items-center justify-center shrink-0">
        {workspace?.logo_url || tenant.logo ? (
          ...
          <div className="flex items-center gap-2">
            <div className="w-8 h-8 rounded-xl bg-theme-primary text-slate-950 font-black flex items-center justify-center text-xs">
              {displayName.substring(0, 2).toUpperCase()}
            </div>
            <span className="font-bold text-base text-white">{displayName}</span>
          </div>
        )}
      </header>
      ...
      <main className="max-w-md mx-auto w-full my-auto flex-1 flex flex-col justify-center overflow-hidden">
        {isSuccess ? (
          <div className="bg-navy-900/90 border border-theme-primary/30 rounded-3xl p-5 sm:p-6 text-center space-y-4 shadow-2xl animate-in zoom-in-95 duration-200 my-auto">
            <div className="w-14 h-14 bg-gradient-to-tr from-theme-primary to-theme-primary-light text-navy-950 rounded-2xl flex items-center justify-center mx-auto shadow-lg">
            ...
            <div className="space-y-1.5">
              ...
              <p className="text-xs text-slate-300 leading-relaxed">
                ...
                <strong className="text-theme-primary">{location}</strong>. Em breve nossa equipe entrará em contato via WhatsApp.
              </p>
            </div>
            ...
            <div className="pt-3 border-t border-navy-800/80 flex flex-col gap-2.5">
              <button
                onClick={handleReset}
                className="w-full py-2.5 px-4 rounded-xl text-xs font-bold bg-navy-800 text-theme-primary hover:bg-navy-700 border border-theme-primary/20 transition-all cursor-pointer"
              >
          ...
          <div className="bg-navy-900/90 backdrop-blur-md border border-theme-primary/20 rounded-3xl p-4 sm:p-5 shadow-2xl space-y-3 max-h-[92vh] sm:max-h-[620px] flex flex-col justify-between overflow-y-auto no-scrollbar">
            <div className="text-center space-y-1 shrink-0">
              <div className="inline-flex items-center justify-center p-2 rounded-xl bg-theme-primary/10 border border-theme-primary/20 text-theme-primary mb-0.5">
                <HeartHandshake className="w-6 h-6" />
              </div>
              <h1 className="text-xl font-black text-white tracking-tight">
                Convide o {displayName}
              </h1>
            ...
            <form onSubmit={handleSubmit} className="space-y-2.5 flex-1 flex flex-col justify-between">
              <div className="space-y-2">
                <div className="space-y-0.5">
                  ...
                  <input
                    ...
                    required
                    className="w-full px-3 py-2 rounded-xl border border-navy-700 bg-navy-950/80 text-slate-100 text-xs placeholder:text-slate-500 focus:outline-none focus:ring-2 focus:ring-theme-primary/50"
                  />
                </div>
                ...
                <div className="space-y-0.5">
                  ...
                  <select
                    ...
                    className="w-full px-3 py-2 rounded-xl border border-navy-700 bg-navy-950 text-slate-100 text-xs focus:outline-none focus:ring-2 focus:ring-theme-primary/50"
                  >
                ...
                <div className="grid grid-cols-2 gap-2">
                  <div className="space-y-0.5">
                    ...
                    <input
                      ...
                      className="w-full px-2.5 py-1.5 rounded-xl border border-navy-700 bg-navy-950 text-slate-100 text-xs focus:outline-none focus:ring-2 focus:ring-theme-primary/50"
                    />
                  </div>
                ...
                <div className="space-y-0.5">
                  ...
                  <textarea
                    ...
                    className="w-full px-3 py-1.5 rounded-xl border border-navy-700 bg-navy-950/80 text-slate-100 text-xs placeholder:text-slate-500 focus:outline-none focus:ring-2 focus:ring-theme-primary/50 resize-none"
                  />
                </div>
              </div>
              ...
              <button
                ...
                className="w-full py-3 px-4 rounded-xl text-xs font-extrabold bg-gradient-to-r from-theme-primary to-theme-primary-dark text-slate-950 hover:brightness-110 shadow-lg active:scale-[0.99] transition-all flex items-center justify-center gap-2 cursor-pointer disabled:opacity-50 shrink-0 mt-1"
              >
                {isSubmitting ? (
      ...
      <footer className="max-w-md mx-auto w-full pt-1 pb-1 text-center text-[10px] text-slate-400 shrink-0">
        <p>&copy; {new Date().getFullYear()} {displayName} — Todos os direitos reservados</p>
      </footer>
    </div>
  );
}
